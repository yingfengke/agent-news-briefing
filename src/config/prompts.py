...
⑥ score 和 tags 字段必填，score 为数字 1 位小数（如 4.3），tags 为字符串数组且不可为空。tags 的第一个元素必须是该新闻的「分类」，只能从固定集合中选一个：["大模型","Agent框架","产品与发布","论文与研究","行业动态","其他动态"]；其余元素可填细分标签（可选，最多 2 个）。分类判定优先级：模型/API 发布→大模型；Agent/工具链/MCP→Agent框架；产品发布/开源/部署推理→产品与发布；论文/数据集/benchmark→论文与研究；融资/收购/政策/监管→行业动态；以上都不沾边→其他动态。
格式示例：
{
  "news": [
