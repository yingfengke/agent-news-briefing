...
# 导入动态爬虫模块（Phase 1 新增）
try:
    from advanced_crawler import run_all_crawlers as run_advanced_crawlers
...
MAX_PER_SOURCE = {
    # 中文媒体
    "机器之心": 3, "量子位": 3, "InfoQ中文": 3, "阿里云开发者": 3, "腾讯云开发者": 3,
    # 前沿论文
    "HuggingFace论文": 3, "ArXiv AI": 3, "PapersWithCode": 3,
    # 核心框架
    "LangChain": 3, "OpenAI": 3, "Google AI": 3, "Anthropic": 3,
    "Meta AI": 3, "LlamaIndex": 3,
    # 全球社区
...
def main():
    ...
    print(f"  RSS 源: {len(RSS_SOURCES)} 个 (4 大类) | 模型: {MODEL_NAME}")
    print("=" * 60)
    ...
    # ---- 补充：动态网页爬虫（Playwright 驱动） ----
    print(f"\n  ── 动态网页爬虫 ──")
    if ADVANCED_CRAWLER_AVAILABLE:
        try:
            crawler_items = run_advanced_crawlers()
            all_items.extend(crawler_items)
            print(f"  爬虫模块贡献 {len(crawler_items)} 条")
        except Exception as e:
            print(f"  [降级] 爬虫模块异常: {e}")
            # 降级到旧的 scrape_chinese_news
            print(f"  ── 降级：旧版中文爬虫 ──")
            chinese_items = scrape_chinese_news()
            all_items.extend(chinese_items)
    else:
        print(f"  [降级] Playwright 不可用，使用旧版爬虫")
        chinese_items = scrape_chinese_news()
        all_items.extend(chinese_items)
    print(f"  总计 {len(all_items)} 条（RSS + 爬虫）")

    # ---- 传给 AI 的新闻量控制 ----
    # 按语言分桶，确保中英文均有代表，不被截断
    CHINESE_SOURCES = {"机器之心", "量子位", "InfoQ中文", "阿里云开发者", "腾讯云开发者", "V2EX AI", "量子位爬虫", "机器之心爬虫", "魔搭社区爬虫", "腾讯云开发者爬虫", "IT之家AI"}
