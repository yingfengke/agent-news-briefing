...
async function checkTodayRan() {
  ...
  const url = `https://api.github.com/repos/yingfengke/tech-breakfast/actions/runs?per_page=10&status=completed`;
...
async function triggerGitHubActions() {
  const token = GITHUB_TRIGGER_TOKEN;
  const url =
    "https://api.github.com/repos/yingfengke/tech-breakfast/actions/workflows/daily-briefing.yml/dispatches";
...
async function triggerViaPush() {
  try {
    ...
    const headResp = await fetch(
      "https://api.github.com/repos/yingfengke/tech-breakfast/git/ref/heads/main",
      {
    ...
    const createResp = await fetch(
      "https://api.github.com/repos/yingfengke/tech-breakfast/git/commits",
      {
        method: "POST",
